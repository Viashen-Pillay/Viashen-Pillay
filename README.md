# Hi there 👋 I'm Viashen Pillay

## 🚀 About Me
Passionate developer who wants to learn as much as I can. I am interested in AI, Web development and App Development.

---

## 📁 Projects I've Done

### 🌿 BC Wild Watch
A wildlife monitoring and tracking application.
- **Repository**:  bc-wildwatch-v2 (https://github.com/Sikhwendigo/bc-wildwatch-v2.git)

### 🌿 Hero Database
A Hero databased.
- **Repository**:  SuperHero DataBase (https://github.com/Sikhwendigo/PRG282.git)

### 🌿 The Smart Event Management & Ticketing Platform
A ticket tracking system.
- **Repository**:  Smart Events (https://github.com/MASANAMATHONSI/WPR381-Project.git)


### 📄 My CV
View my professional experience and skills.
- **Repository**: [CV](https://github.com/Viashen-Pillay/CV.git)

---

## 🛠️ Tech Stack
- [Add your technologies]
- Example: Python, JavaScript, React, etc.

## 📫 Connect With Me
- GitHub: [@Viashen-Pillay](https://github.com/Viashen-Pillay)
- LinkedIn: https://www.linkedin.com/in/viashen-pillay-413835224/
- Email: viashengp@gmail.com

---

⭐️ From [Viashen-Pillay](https://github.com/Viashen-Pillay)
